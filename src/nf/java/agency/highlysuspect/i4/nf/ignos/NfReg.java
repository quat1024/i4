...
public class NfReg<T> extends Reg<T> {
	...
	protected Map<Id, Supplier<? extends T>> creators = new HashMap<>();
	...
	public void doRegister(RegisterEvent e) {
		...
		if(creators == null) throw new IllegalStateException("NfReg#force called twice for " + registry.key());
		creators.forEach((id, creator) -> {
			...
			//I want to call shutLatches after creating the object though
